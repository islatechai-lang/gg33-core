...
type SectionTab = 'blueprint' | 'insights' | 'wheel' | 'placements';

export default function BirthChartPage() {
  const { dbUser } = useAuth();
  const [activeTab, setActiveTab] = useState<SectionTab>('blueprint');
  ...
  const tabItems: { id: SectionTab; label: string; icon: any; proOnly?: boolean }[] = [
    { id: 'blueprint', label: 'Big 3 & Blueprint', icon: Crown },
    { id: 'insights', label: 'Life Insights', icon: Zap, proOnly: true },
    { id: 'wheel', label: 'Natal Wheel', icon: Compass, proOnly: true },
    { id: 'placements', label: 'Placements & Aspects', icon: Layers, proOnly: true },
  ];
        ...
        <div className="w-full max-w-6xl mx-auto space-y-6">
          ...
          <div className="relative rounded-3xl p-6 sm:p-8 bg-gradient-to-br from-zinc-950 via-zinc-900 to-amber-950/20 border border-zinc-800 shadow-2xl overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  {isPro ? (
                    ...
                    <Badge className="bg-amber-500/10 border-amber-500/30 text-amber-400 text-xs flex items-center gap-1">
              ...
              {/* Big 3 Quick Pill */}
              <div className="flex items-center gap-3 bg-zinc-950/80 p-3 rounded-2xl border border-zinc-800/80 backdrop-blur-md self-start md:self-auto shadow-lg">
            ...
            {tabItems.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  <span>{tab.label}</span>
                  {!isPro && tab.proOnly && (
                    <Lock className={`w-3 h-3 ${isActive ? 'text-zinc-900' : 'text-zinc-500'}`} />
                  )}
                </button>
              );
            })}
          ...
          {activeTab === 'blueprint' && (
            <div className="space-y-6 animate-fade-in">
              ...
              {isPro ? (
                ...
                /* Free User Pro Upgrade Callout */
                <Card className="w-full bg-zinc-950/95 border-2 border-amber-500/40 backdrop-blur-2xl shadow-2xl p-6 sm:p-8 text-center space-y-6">
                  <div className="w-14 h-14 mx-auto rounded-2xl bg-gradient-to-br from-amber-500 to-amber-600 flex items-center justify-center shadow-lg shadow-amber-500/20">
                    <Crown className="w-7 h-7 text-zinc-950" />
                  </div>
                  <div className="space-y-2">
                    <Badge className="bg-amber-500/10 text-amber-400 border-amber-500/30 text-xs px-2.5 py-0.5 font-bold">
                    ...
                    <p className="text-xs sm:text-sm text-zinc-400 max-w-md mx-auto leading-relaxed">
                      Upgrade to GG33 Pro to access the interactive chart wheel, superpower breakdown, love & career blueprints, and complete placement table.
                    </p>
                  </div>
                  <Button
                    size="lg"
                    className="w-full max-w-md mx-auto bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-zinc-950 font-extrabold text-sm h-11 rounded-xl shadow-lg shadow-amber-500/25 transition-all cursor-pointer"
                    onClick={() => setShowUpgradeModal(true)}
                  >
                    <Sparkles className="w-4 h-4 mr-2 text-zinc-950" />
                    Unlock Full Birth Chart ($35/mo)
                  </Button>
                </Card>
              )}
            </div>
              ...
              {!isPro ? (
                <Card className="w-full bg-zinc-950/95 border-2 border-amber-500/40 backdrop-blur-2xl shadow-2xl p-6 sm:p-8 text-center space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <h2 className="text-xl sm:text-2xl font-extrabold text-zinc-100">
                      Unlock Life Mastery & Esoteric Insights
                    </h2>
                    <p className="text-xs sm:text-sm text-zinc-400 max-w-md mx-auto leading-relaxed">
                      Discover your planetary superpowers, karmic shadow challenge & breakthrough solution, love non-negotiables, and Midheaven wealth path.
                    </p>
                  </div>
                  <Button
                    size="lg"
                  ...
                  >
                    <Sparkles className="w-4 h-4 mr-2 text-zinc-950" />
                    Unlock Life Insights with Pro
                  </Button>
                </Card>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* 1. Superpowers Card */}
                  <Card className="bg-zinc-950/80 border-zinc-800 backdrop-blur-xl p-6 space-y-4 shadow-xl">
          ...
          {/* TAB 3: Natal Wheel & Elements (Pro Only) */}
          {activeTab === 'wheel' && (
            <div className="space-y-6 animate-fade-in">
              {!isPro ? (
                <Card className="w-full bg-zinc-950/95 border-2 border-amber-500/40 backdrop-blur-2xl shadow-2xl p-6 sm:p-8 text-center space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <h2 className="text-xl sm:text-2xl font-extrabold text-zinc-100">
                      Unlock Interactive Natal Wheel
                    </h2>
                    <p className="text-xs sm:text-sm text-zinc-400 max-w-md mx-auto leading-relaxed">
                      Interact with your SVG natal chart wheel, inspect aspect chords across the 12 houses, and explore elemental balances.
                    </p>
                  </div>
                  <Button
                    size="lg"
                ...
                <>
                  ...
                  <div className="space-y-4">
                    <div>
                      <h2 className="text-xl font-bold text-zinc-100 flex items-center gap-2">
                        ...
                        Interactive Natal Wheel & Geometric Aspect Lines
                      </h2>
                      <p className="text-xs text-zinc-400">
                        Click any planet on the wheel or in the inspector to view its exact house, degrees, and aspect connections.
                      </p>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {/* Elements Balance */}
                    <Card className="bg-zinc-950/80 border-zinc-800/80 backdrop-blur-xl p-6 space-y-4 shadow-xl">
                      <div className="flex items-center justify-between">
          ...
          {/* TAB 4: Placements & Aspects (Pro Only) */}
          {activeTab === 'placements' && (
            <div className="space-y-6 animate-fade-in">
              {!isPro ? (
                <Card className="w-full bg-zinc-950/95 border-2 border-amber-500/40 backdrop-blur-2xl shadow-2xl p-6 sm:p-8 text-center space-y-6">
                  <div className="w-14 h-14 mx-auto rounded-2xl bg-gradient-to-br from-amber-500 to-amber-600 flex items-center justify-center shadow-lg shadow-amber-500/20">
                    <Layers className="w-7 h-7 text-zinc-950" />
                  </div>
                  <div className="space-y-2">
                    <Badge className="bg-amber-500/10 text-amber-400 border-amber-500/30 text-xs px-2.5 py-0.5 font-bold">
                      PRO EXCLUSIVE FEATURE
                    ...
                    <h2 className="text-xl sm:text-2xl font-extrabold text-zinc-100">
                      Unlock Full Placements & Aspects Matrix
                    </h2>
                    <p className="text-xs sm:text-sm text-zinc-400 max-w-md mx-auto leading-relaxed">
                      Access all 12 planetary house placements, degrees, retrograde markers, and precise aspect orbs.
                    </p>
                  </div>
                  <Button
                    size="lg"
                  ...
                  >
                    <Sparkles className="w-4 h-4 mr-2 text-zinc-950" />
                    Unlock Placements Table with Pro
                  </Button>
                </Card>
              ) : (
                <>
